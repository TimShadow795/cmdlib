...
local function th(bg,ch,hov,ln,tab,fg,dm,gn,rd,yl,cy,mg,bl,cl) return {bg=bg,chrome=ch,chromeHover=hov,chromeLine=ln,tabActive=tab,fg=fg,dim=dm,green=gn,red=rd,yellow=yl,cyan=cy,magenta=mg,blue=bl,close=cl} end
...
T.Colors={} for k,v in pairs(T.Themes.default) do T.Colors[k]=v end
...
function T.new(c)
  ...
  mkBtn(-168,C.chromeHover,function(p) local l=Instance.new("Frame") l.AnchorPoint=Vector2.new(.5,.5) l.Position=UDim2.new(.5,0,.5,0) l.Size=UDim2.new(0,12,0,2) l.BackgroundColor3=C.fg l.BorderSizePixel=0 l.Parent=p end,function() if w.Size.Y.Offset>100 then TW:Create(w,TweenInfo.new(.2),{Size=UDim2.new(0,s.W,0,36)}):Play() else TW:Create(w,TweenInfo.new(.2),{Size=UDim2.new(0,s.W,0,s.H)}):Play() end end)
  mkBtn(-112,C.chromeHover,function(p) local outer=Instance.new("Frame") outer.AnchorPoint=Vector2.new(.5,.5) outer.Position=UDim2.new(.5,0,.5,0) outer.Size=UDim2.new(0,10,0,10) outer.BackgroundColor3=C.fg outer.BorderSizePixel=0 outer.Parent=p local inner=Instance.new("Frame") inner.AnchorPoint=Vector2.new(.5,.5) inner.Position=UDim2.new(.5,0,.5,0) inner.Size=UDim2.new(0,6,0,6) inner.BackgroundColor3=C.chrome inner.BorderSizePixel=0 inner.Parent=outer end,function() if w.Size.X.Offset<=s.W+10 then TW:Create(w,TweenInfo.new(.2),{Size=UDim2.new(0,s.W+160,0,s.H+100)}):Play() else TW:Create(w,TweenInfo.new(.2),{Size=UDim2.new(0,s.W,0,s.H)}):Play() end end)
  local closeBtn=mkBtn(-56,C.close,function(p) local a=Instance.new("Frame") a.AnchorPoint=Vector2.new(.5,.5) a.Position=UDim2.new(.5,0,.5,0) a.Size=UDim2.new(0,13,0,2) a.BackgroundColor3=C.fg a.BorderSizePixel=0 a.Rotation=45 a.Parent=p local b=Instance.new("Frame") b.AnchorPoint=Vector2.new(.5,.5) b.Position=UDim2.new(.5,0,.5,0) b.Size=UDim2.new(0,13,0,2) b.BackgroundColor3=C.fg b.BorderSizePixel=0 b.Rotation=-45 b.Parent=p end,function() TW:Create(w,TweenInfo.new(.2),{Size=UDim2.new(0,s.W,0,0)}):Play() task.wait(.25) s:Destroy() end)
  for _,e in ipairs(btns) do if e.btn==closeBtn then e.isClose=true end end
  UIS.InputBegan:Connect(function(input) local isM=input.UserInputType==Enum.UserInputType.MouseButton1 local isT=input.UserInputType==Enum.UserInputType.Touch if not (isM or isT) then return end local pos=isT and input.Position or UIS:GetMouseLocation() for _,e in ipairs(btns) do local b=e.btn if b.Parent and b.Visible then local ap=b.AbsolutePosition local as=b.AbsoluteSize if pos.X>=ap.X and pos.X<=ap.X+as.X and pos.Y>=ap.Y and pos.Y<=ap.Y+as.Y then fire(e) return end end end end)
  local dragging,ds,dp=false,nil,nil
  tb.InputBegan:Connect(function(input) if input.UserInputType~=Enum.UserInputType.MouseButton1 and input.UserInputType~=Enum.UserInputType.Touch then return end local p=UIS:GetMouseLocation() local tba=tb.AbsolutePosition local tbs=tb.AbsoluteSize if p.X>tba.X+tbs.X-190 then return end dragging=true ds=input.Position dp=w.Position input.Changed:Connect(function() if input.UserInputState==Enum.UserInputState.End then dragging=false end end) end)
  UIS.InputChanged:Connect(function(input) if not dragging then return end if input.UserInputType~=Enum.UserInputType.MouseMovement and input.UserInputType~=Enum.UserInputType.Touch then return end local d=input.Position-ds w.Position=UDim2.new(dp.X.Scale,dp.X.Offset+d.X,dp.Y.Scale,dp.Y.Offset+d.Y) end)
  ...
  local tab=Instance.new("Frame") tab.Position=UDim2.new(0,8,0,6) tab.Size=UDim2.new(0,200,1,-6) tab.BackgroundColor3=C.tabActive tab.BorderSizePixel=0 tab.Parent=tbb Instance.new("UICorner",tab).CornerRadius=UDim.new(0,6) tag(tab,"tabActive")
  ...
  local bd=Instance.new("Frame") bd.Position=UDim2.new(0,0,0,72) bd.Size=UDim2.new(1,0,1,-72) bd.BackgroundColor3=C.bg bd.BorderSizePixel=0 bd.Parent=w Instance.new("UICorner",bd).CornerRadius=UDim.new(0,8) tag(bd,"bg")
  ...
  function s:SetTheme(name)
    ...
    for _,e in ipairs(self._chrome) do if e.o and e.o.Parent then if e.o:IsA("TextLabel") or e.o:IsA("TextBox") then e.o.TextColor3=C[e.k] else e.o.BackgroundColor3=C[e.k] end end end
  ...
  function s:AddBuiltins()
    ...
    reg({Name="colors",Description="list themes or switch theme",Usage="colors [name]",Callback=function(x,a)
      ...
      if a[1] then if x:SetTheme(a[1]) then x:Write("theme -> "..a[1],C.green) else x:Write("themes: "..table.concat(names,", "),C.dim) end
      else x:Write("themes:",C.yellow) for _,n in ipairs(names) do x:Write("  "..n,C.fg) end x:Write("usage: colors <name>",C.dim) end
    end})
    reg({Name="color",Description="alias colors",Usage="color <name>",Callback=function(x,a) if not a[1] then x:Write("usage: color <name>",C.yellow) return end if x:SetTheme(a[1]) then x:Write("theme -> "..a[1],C.green) end end})
    ...
    reg({Name="resetcommands",Description="restore all builtin commands",Callback=function(x) x:WipeCommands() x.Builtins={} x:AddBuiltins() x:Write("builtins restored",C.green) end})
